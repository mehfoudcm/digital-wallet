...
print "Please type the csv filename of transaction history to be used (default is batch_payment.csv):"
...
ncount = newpayment['id1'].count()
hcount = paymentlist['id1'].count()
...
#use z score cap of 44.98, anything over within top 2.275% should be verified
#pricecap = 44.98
else:
    pricecap = 44.98
...
timing=newpayment.id2.value_counts().reset_index().rename(columns={'index': 'id2', 'id2': 'countpersec'})
...
for row in nodeset1:
    if g.has_node(row[0]) and g.has_node(row[1]):
        try:
            d = nx.dijkstra_path_length(g,row[0],row[1])
            #d1 = len(nx.shortest_path((g,row[0],row[1]))-1
            pathlength.append(d)
